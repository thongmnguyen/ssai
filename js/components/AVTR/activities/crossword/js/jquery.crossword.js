...
(function($) {
	$.fn.crossword = function(entryData) {
		...
		this.after(
			'<div id="puzzle-clues"><h2>Across</h2><ol id="across"></ol><h2>Down</h2><ol id="down"></ol></div>'
		);
		...
		var puzInit = {
			...
			calcCoords: function() {
				...
				for (var i = 0, p = entryCount; i < p; ++i) {
					...
					for (
						var x = 0, j = puzz.data[i].answer.length;
						x < j;
						++x
					) {
						entries[i].push(x);
			...
			buildEntries: function() {
				...
				for (var x = 1, p = entryCount; x <= p; ++x) {
					...
					for (var i = 0; i < entries[x - 1].length; ++i) {
						...
						if (x > 1) {
							if (
								puzz.data[x - 1].position ===
								puzz.data[x - 2].position
							) {
								hasOffset = true;
							}
						}
				...
				for (var i = 1, p = entryCount; i < p; ++i) {
					...
					if (!$(".entry-" + i + ":eq(0) span").length) {
						$groupedLights
							.eq(0)
							.append(
								"<span>" + puzz.data[i].position + "</span>"
							);
					}
				}
		...
		var nav = {
			...
			updateByEntry: function(e, next) {
				...
				if (e.keyCode === 9 || next) {
					...
					activeClueIndex =
						activeClueIndex === clueLiEls.length - 1
							? 0
							: ++activeClueIndex;
					...
					activePosition = $(clueLiEls[activeClueIndex]).data(
						"position"
					);
				} else {
					activeClueIndex =
					...
					clue = $(
						clueLiEls + "[data-position=" + activePosition + "]"
					);
		...
		var util = {
			...
			getActivePositionFromClassGroup: function(el) {
				...
				if (classes.length > 1) {
					...
					e1Cell = $(
						".position-" + classes[0].split("-")[1] + " input"
					).index(el);
					e2Cell = $(
						".position-" + classes[1].split("-")[1] + " input"
					).index(el);
