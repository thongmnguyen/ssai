...
define(["module/HUD"], function(HUD) {
	...
	var _Aliens = function(configuration) {
		...
		function _createAllienGroup() {
			...
			_tween = _game.add
				.tween(_alienGroup)
				.to({ x: 200 }, 2000, _easing, true, 0, 1000, true);
		}
		...
		var _fireBullet = function() {
			...
			if (_bullet && _livingAlien.length > 0) {
				...
				_randomAlienIndex = _game.rnd.integerInRange(
					0,
					_livingAlien.length
				);
				...
				if (_shooter) {
					...
					_game.physics.arcade.moveToObject(
						_bullet,
						_playerShip,
						_bulletSpeed
					);
		...
		return {
			...
			startShooting: function() {
				_shootingEvent = _game.time.events.loop(
					_firingTime,
					_fireBullet,
					this
				);
